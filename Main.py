...
def callback_handler(call):
    ...
    if call.data == "top_menu":
        ...
        if not top_list: res_text += "<i>Hozircha faollar yo'q.</i>"
        else:
            for i, (name, spent) in enumerate(top_list): res_text += f"{i+1}. {name} — {spent:,} so'm\n"
        bot.edit_message_text(res_text, cid, mid, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Ortga", callback_data="top_menu")), parse_mode="HTML")
        ...
        stars_list = [("50", 10250), ("75", 15375), ("100", 20500), ("150", 30750), ("250", 51250), ("350", 71750), ("500", 102500), ("750", 153750), ("1000", 205000), ("1500", 307500), ("2500", 512500), ("5000", 1025000)]
        btns = [types.InlineKeyboardButton(f"⭐ {s[0]} - {int(s[1]):,} so'm", callback_data=f"ord_STARS_{s[0]}_{s[1]}") for s in stars_list]
        markup.add(*btns).row(types.InlineKeyboardButton("⬅️ Ortga", callback_data="back_to_main"))
        bot.edit_message_text(text, cid, mid, reply_markup=markup, parse_mode="HTML")

    elif call.data == "buy_gift":
        text = f"<b>🎁 Telegram Gift xarid qilish</b>\n\nℹ️ Yuborish {MY_USERNAME} orqali amalga oshiriladi"
        markup = types.InlineKeyboardMarkup(row_width=2)
        gifts = [("💝", 3000), ("🧸", 3000), ("🎁", 5000), ("🌹", 5000), ("🎂", 10000), ("💐", 10000), ("🚀", 10000), ("🍾", 10000), ("🏆", 20000), ("💍", 20000), ("💎", 20000), ("🎈", 10000)]
        ...
        if order:
            ...
            conn = sqlite3.connect(DB_NAME); cursor = conn.cursor()
            cursor.execute("UPDATE users SET orders = orders + 1, total_spent = total_spent + ?, last_order_date = ? WHERE user_id = ?", (order['price'], now, uid))
            conn.commit(); conn.close()
        ...
        bot.edit_message_text(f"📊 <b>Statistika</b>\n\n<blockquote>▫️ Buyurtmalar: {user['orders']} ta\n▫️ Sarf: {user['total_spent']:,} so'm\n▫️ O'rningiz: {rank}</blockquote>", cid, mid, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Ortga", callback_data="back_to_main")), parse_mode="HTML")
    ...
    if state == "WAITING_STARS_AMOUNT" and message.text.isdigit():
        ...
        try:
            ...
            if res.get("ok"): exists = True
                ...
                tg_page = requests.get(f"https://t.me/{clean_name}", headers={'User-Agent': 'Mozilla/5.0'}, timeout=5).text
        ...
        res_text = (f"🏪 <b>Buyurtma yaratildi</b>\n\n"
                    f"<blockquote>■ {order['type']} ({order['name']})\n"
                    f"■ @{clean_name}\n"
                    f"■ {order['price']:,} so'm</blockquote>")
        markup = types.InlineKeyboardMarkup()
